...
def pull_model(project, logic, use_case, facts, inferences, unknowns, raw_score=None, pull_box=None):
    """Translate enriched evidence into a demand-first PULL hypothesis."""
    box = mapping(pull_box)
    job = box.get("project") or project
    unavoidable = box.get("u_unavoidable") or (facts[0] if facts else "尚未找到客户必须完成该工作的直接证据")
    limitations = box.get("l_limitations") or (inferences[0] if inferences else "现有工作流、系统覆盖和失败成本待验证")
    leverage = box.get("l_leverage") or use_case or "尚未证明软件能够改善客户结果"
    combined = f"{logic or ''} {leverage}".lower()
    if not use_case or str(use_case).lower().startswith("unknown"):
        mode, verdict = "Needs Validation", "尚未形成软件 PULL"
    elif any(k in combined for k in ("多站点", "数据", "资产", "远程", "报警", "报表", "管理", "诊断", "预测")):
        mode = "Integrate" if any(k in combined for k in ("scada", "dcs", "平台", "系统", "接入")) else "Extend"
        verdict = "可验证的软件 PULL"
    else:
        mode, verdict = "Needs Validation", "软件增量价值待验证"
    scores = mapping(raw_score)
    total = scores.get("total")
    return {
        "job": text(job), "unavoidable": text(unavoidable), "limitations": text(limitations),
        "instrument_supply": "用在线仪表提供连续、可信、可追溯的现场数据；具体测量点与数据质量需按项目验证",
        "software_supply": text(leverage), "delivery_mode": mode, "verdict": verdict,
        "score": total, "score_detail": scores,
        "validation_question": unknowns[0] if unknowns else "客户当前如何完成这项工作，最耗时或最容易失败的环节是什么？",
    }
...
def industrial_profiles(raw):
    ...
    for lead in raw.get("leads", []):
        ...
        profiles.append({
            ...
            "pull": pull_model(project.get("project_name") or signal.get("opportunity"), signal.get("logic"), signal.get("potential_ims_use_case"), facts, inferences, unknowns, signal.get("pull_score"), signal.get("pull_box")),
...
def municipal_profiles(raw):
    ...
    for lead in raw.get("leads", []):
        ...
        profiles.append({
            ...
            "penetration": penetration,
            "pull": pull_model(project.get("name") or lead.get("name"), next((f.get("iMS_impact") for f in facts if f.get("iMS_impact")), ""), lead.get("iMS_use_case"), [f.get("fact") for f in facts if f.get("fact")], [], account.get("missing_roles", []) or raw.get("missing_roles", [])),
...
<header><h1>Water Clay · Demand & Software PULL</h1><p>从客户必须完成的工作出发，验证仪表数据与 iMS 软件能否形成可量化的结果</p></header><div class="toolbar"><input id="q" aria-label="搜索" placeholder="搜索客户、Job、问题或地区"><select id="market" aria-label="市场"><option value="">全部市场</option><option>工业</option><option>市政</option></select><select id="priority" aria-label="优先级"><option value="">全部优先级</option><option>P0</option><option>P1</option><option>P2</option><option>A</option><option>B</option><option>C</option></select></div>
...
</script></body></html>'''.replace("[q,market,priority].forEach", '''function renderDetail(p){if(!p)return;const x=p.pull||{};detail.innerHTML=`<section class="hero"><span class="tag">${esc(p.market)}</span><span class="tag">${esc(x.delivery_mode)}</span><h2>${esc(p.account)}</h2><p>${esc(x.verdict)} · ${esc(p.project)}</p></section><div class="kpis"><section class="panel kpi"><small>PULL 总分</small><b>${esc(x.score)}</b></section><section class="panel kpi"><small>Supply 路径</small><b>${esc(x.delivery_mode)}</b></section><section class="panel kpi"><small>判断</small><b>${esc(x.verdict)}</b></section></div><div class="columns"><section class="panel wide"><h3>P · 客户要完成的 Job</h3><p>${esc(x.job)}</p></section><section class="panel"><h3>U · 为什么不可回避</h3><p>${esc(x.unavoidable)}</p></section><section class="panel"><h3>L · 当前方案的限制</h3><p>${esc(x.limitations)}</p></section><section class="panel"><h3>仪表 Supply</h3><p>${esc(x.instrument_supply)}</p></section><section class="panel wide"><h3>L · iMS / 软件 Leverage</h3><p>${esc(x.software_supply)}</p><p><b>交付判断：</b>${esc(x.delivery_mode)}</p></section><section class="panel"><h3>最关键验证问题</h3><p class="unknown">${esc(x.validation_question)}</p></section><section class="panel wide"><h3>客户行为证据</h3>${bullets(p.behavior)}</section><section class="panel sources"><h3>证据入口</h3>${arr(p.sources).slice(0,8).map(u=>`<a href="${esc(u)}" target="_blank" rel="noreferrer">${esc(u)}</a>`).join('')||'<p class="muted">源文件未提供 URL</p>'}</section></div>`} [q,market,priority].forEach''')
