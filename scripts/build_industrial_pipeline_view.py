...
def contacts_for(item, contacts):
    result=[]
    for cid in item.get("contact_ids") or []:
        if cid in contacts:
            contact=safe(json.loads(json.dumps(contacts[cid], ensure_ascii=False))); contact["name"]=mask_name(contact.get("name")); contact.pop("email", None); result.append(contact)
    return result
