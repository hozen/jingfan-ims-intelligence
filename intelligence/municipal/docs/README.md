...
| `iMS_GTM_Public_Signal_Radar_prompt_v3.4_latest_20260917.md` | v3.4.1（完整版 + latest.json 累积快照恢复） | 2026-09-17 | **当前市政线生效版本（job_14c6b43e）**。基于 v3.4 反转 latest.json 条款：每次运行**同步更新 `intelligence/municipal/latest.json`**（累积快照——反映当前所有仍有效线索，包含全部收集到的数据；跨日累积保留 first_seen/continuity，失效或晋级线索移出 opportunities[] 并标注处置），不再生成 PDF；qc_checklist 以 `latest_json_updated` 取代 `no_latest_json_generated`。2026-09-17 用户指示："每次运行任务时同步更新 latest.json，反映当前所有依旧有效的线索数据，文件包含所有收集到的数据" |
...
| `iMS_GTM_Public_Signal_Radar_prompt_v3.7_20260913.md` | v3.7（v3.6 × 集团层面切入法） | 2026-09-13 | **当前全量线最新版**。在 v3.6 基础上新增**集团/战略层面切入法（Group-Level Entry）**：对大型水务/环境集团（外资：威立雅/苏伊士；央企国企：北控/首创/粤海/重庆水务等）从集团战略、数智化平台、新建扩建、运营数字化尝试与痛点四个层面系统切入，发现跨项目提前量线索；Radar 2 扩展集团化主体维度；qc_checklist 新增 group_level_entry_applied；opportunities 可选增加 lead_type（运营型/新建型/改造型）。源自 2026-09-13 威立雅中国战略客户专题实践（P1×1/P2×4，5 条机会全部集团切入产出） |
...
- **v3.4（2026-09-11/12）**：Stage 1 Discovery + Stage 2 Qualification 三段管线继续演进，为 Company Agent 交接提供更细的阶段化信息。
- **v3.4.1（2026-09-17）**：用户指示恢复 latest.json——每次运行同步更新 `intelligence/municipal/latest.json` 作为**当前所有仍有效线索的累积快照**（含全部收集数据；跨日累积、标注 first_seen/continuity；失效/晋级线索移出 opportunities[] 并注明处置）。注意：**此恢复只针对市政线 job_14c6b43e 的当前生效版**；全量线 v3.2_full/v3.6/v3.7 的 latest.json 语义（覆盖更新）不受影响。若未来要改为"不维护 latest 快照"，需用户明确指示撤销本变更。
...
- **v3.7（2026-09-13）**：在 v3.6 之上新增**集团/战略层面切入法（Group-Level Entry）**——源于威立雅中国战略客户专题实践：单一项目/招标关键词只能发现已进入设计/招标阶段的项目，提前量有限；从集团主体（战略/数智化平台/新建扩建/运营数字化尝试与痛点/生态竞争）切入可发现跨项目 3-18 个月提前量线索。新增：目标集团清单（外资/央企国企/跨区域集团）、四类切入源、线索下沉规则（集团信号必须下沉到具体项目/地区/城市/水厂，禁止停留在集团层面泛化）、lead_type 标注；qc_checklist 新增 group_level_entry_applied。
