...
**latest.json 语义（2026-09-17 用户指示新增，硬性）**：每次运行后同步覆盖更新 intelligence/municipal/latest.json，作为「当前全部有效线索」的累积快照。包含：scanner_summary（累计口径）、top_5_match、opportunities[]（仅仍处于战略规划→方案阶段的线索，含 first_seen/continuity）、background_monitoring[]（招采阶段累积，含追溯信息）、strategic_accounts[]（含 first_seen/连续性），以及 industry/ecosystem/policy/competitive_triggers[] 的全部有效条目；已失效或阶段晋级的线索移出 opportunities[]，转入 background_monitoring[] 或标注处置并在快照中说明。latest.json 必须包含所有收集到的数据，不得只放当日新增。
