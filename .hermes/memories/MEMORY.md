...
§
Baidu API key (bce-v3/ALTAK-2PWQsNGDWclsFwZxkQKI1/...) is for BCE v3 authentication, NOT for direct Bearer token use. Qianfan API (qianfan.baidubce.com) signature validation fails with this key format. search.bce.baidu.com DNS fails; use qianfan.baidubce.com. Baidu search skill from ClawHub available at ~/.hermes/skills/baidu-search/ but requires Qianfan console API key to work.
§
Camoufox is pre-installed on this system. If user says a tool is already installed, don't try to verify/reinstall - just use it directly. Camoufox works through Hermes browser_* tools (browser_navigate, etc.) - it handles the routing automatically. The camoufox Python venv is at ~/.camoufox-venv/, and the Firefox binary is at /home/agentuser/.cache/camoufox/camoufox.
§
